...
;
OPENFCB	XRA	A	;clear the record number byte at TFCB+32
	STA	TFCB+32
	LXI	D,TFCB
	JMP	OPEN
;
;   Routine to close a file. (DE) points to TFCB.
;
CLOSE	MVI	C,16
	JMP	ENTRY1
;
...
DECODE	CALL	CONVFST	;convert filename.
	...
	ORA	A
	JNZ	SYNERR
	LXI	H,TFCB+1	;convert number now.
...
DIRECT	CALL	CONVFST	;convert file name.
	...
	LXI	H,TFCB+1	;was any file indicated?
	MOV	A,M
	CPI	' '
	JNZ	DIRECT2
...
SAVE	CALL	DECODE	;get numeric number that follows SAVE.
	...
	LXI	D,TFCB	;now delete this file.
	PUSH	D
	CALL	DELETE
	POP	D
...
SAVE1	MOV	A,H	;done yet?
	...
	CALL	DMASET
	LXI	D,TFCB	;write out this sector now.
	CALL	WRTREC
...
;
SAVE2	LXI	D,TFCB	;now close the file.
	CALL	CLOSE
...
RENAME	CALL	CONVFST	;convert first file name.
	...
	LXI	H,TFCB	;yes, move this name into second slot.
	LXI	D,TFCB+16
	MVI	B,16
	CALL	HL2DE
...
RENAME2	MOV	M,B;	reset as per the first file specification.
	XRA	A
	STA	TFCB	;clear the drive byte of the fcb.
...
UNKNOWN	CALL	VERIFY	;check for valid system (why?).
	LDA	TFCB+1	;anything to execute?
	CPI	' '
	JNZ	UNKWN1
...
;
UNKWN1	LXI	D,TFCB+9	;an extension specified?
	LDAX	D
	CPI	' '
...
UNKWN3	PUSH	H
	...
	LXI	D,TFCB	;and read the next record.
	CALL	RDREC
...
GETBACK1:CALL	CONVFST	;convert first name in (FCB).
	LDA	TFCB+1	;if this was just a drive change request,
...
USERDMA	DW	TBUFF	;user's dma address (defaults to 80h).
;
...
GOCPM:               
            ...
            SHLD    0006h ;address field of Jump at 5 to bdos
; 
...
; 
CONST:	    IN      0DEh	;READ CONSOLE STATUS.
	        ANI     01h	    ;LOOK AT KB READY BIT.
	        MVI     A,  0	;SET A=0 FOR RETURN.
	        RZ		        ;NOT READY WHEN NOT 0.
	        CMA		        ;IF READY A=FF.
...
; 
CONOUT:	    IN      0DEh	;READ CONSOLE STATUS.
	        ANI     002 	;IF NOT READY,
	        JZ      CONOUT	;LOOP UNTIL LOW.
...
WRITELOOPA:  PUSH    D
            MVI     D, 015h ; delay a little FF! 40 20!
WRITELOOPA2: 
            NOP
            DCR     D ;decrement counter
            JNZ     WRITELOOPA2 
...
ENDDAT      EQU     $ ;end of data area
DATSIZ      EQU     $-begdat ;	;size of data area
ed from memory image on February 27, 1981
;*
;*                by Clark A. Calkins
;*
;**************************************************************
...
WRITELOOPA:  PUSH    D
            MVI     D, 028h ; delay a little FF! 40 20!
WRITELOOPA2: 
            NOP
            DCR     D ;decrement counter
            JNZ     WRITELOOPA2 
...
WRITELOOP:  PUSH    D
            MVI     D, 080h ; delay a little FF! 60
WRITELOOP2: 
            NOP
            DCR     D ;decrement counter
            JNZ     WRITELOOP2 
